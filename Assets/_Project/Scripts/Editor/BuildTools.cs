...
{
    /// <summary>
    /// WebGL build pipeline: a top-level "Build" menu plus a "Build WebGL" button on
    /// the editor's main toolbar (Unity 6.3 MainToolbarElement API). Every build
    /// first applies the project's WebGL player settings so the output runs from
    /// any static web server (Gzip + JS decompression fallback).
    /// </summary>
    public static class BuildTools
    {
        ...
        static void Build(BuildOptions options)
        {
            ...
            {
                Debug.Log($"WebGL build OK → {Path.GetFullPath(OutputDir)} " +
                          $"({summary.totalSize / (1024f * 1024f):F1} MB in {summary.totalTime.TotalMinutes:F1} min)");
            }
            else
            {
                Debug.LogError($"WebGL build {summary.result}: {summary.totalErrors} error(s) — see console above.");
            }
        }
